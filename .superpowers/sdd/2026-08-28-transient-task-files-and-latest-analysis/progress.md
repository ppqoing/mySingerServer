...
- Task 12：提交 `95753524`。Desktop 删除 Node 本地分析创建、结果、复核和本地删除消费，只保留 PostgreSQL 中心结果；中心结果改为最多 200 行的滑动窗口和最多 8 个内部游标检查点，UI 删除分页按钮并按真实滚动位置整体替换窗口。扫描页不再暴露 Desktop 本地分析入口；中心运行仅在 Completed 时可复核，stale/loading 禁止写操作但保留滚动和有效预览。
- Task 12 fix round：初次聚焦审查为 0 Critical、2 Important。滚动查询风暴以真实 MainWindow 连续请求 RED 固化，修复为 80 毫秒最新窗口防抖，并把运行、类别和组加入去重上下文；迟到成员响应以 R1→R2 交错 RED 固化，Core 在查询前后核对完整作用域，删除成功立即清空窗口并失效在途请求。合并验证 Desktop Core lib 4/4、review_delete 7/7、delete_scope 1/1、cross_phase2 3/3、UI bindings 15/15、window 21/21、offscreen 20/20、fmt/diff-check 全通过；`local_node_e2e` 仍因未提供真实包明确 ignored。
