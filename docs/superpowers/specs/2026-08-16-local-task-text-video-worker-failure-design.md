...
3. 扫描必须读取媒体真实字节：图片依据内容格式解码像素，视频依据容器中视频轨道
   声明的具体编码选择解码器并读取真实帧，不能只依赖文件扩展名或元数据。
4. 修复具体错误源，使正常视频进入 VideoCore 会话管线并返回有效结果；可预期的
   文件级失败必须返回带阶段和原因的 `JobResultMsg`，不得终止 Worker。
5. 重新验证 Compute 包中的真实 `worker.exe`、`videocore.dll` 及其依赖闭包。
...
- 图片读取实际文件字节，以魔数和内容结构识别 JPEG、PNG、WebP 等格式，再调用
  对应格式解码器得到真实像素；扩展名只参与任务分类，不得代替内容识别。
- 视频先由 FFmpeg 解复用容器，从实际视频轨道的 `codecpar->codec_id` 获取编码，
  再由 `av_find_best_stream` / `avcodec_open2` 选择并打开对应解码器。
- H.264、HEVC、VP9 等不同编码不得统一伪装成 H.264，也不得按 MP4、MKV、WebM
  容器名称猜测编码。解码后的真实帧用于抽帧、接触表和特征计算。
- 没有视频轨、系统未包含对应解码器、码流损坏或真实解码失败时，必须报告准确的
  文件级阶段和原因；不能返回伪造成功结果，也不能因此让 Worker 退出。
- 本轮根因修复必须保证 Worker 生产默认路径实际抵达上述 VideoCore 解复用和具体
  编码解码边界，而不是仅验证 VideoCore 独立单元测试。
...
- Worker 真实夹具测试：H.264/MP4、HEVC/MKV、VP9/WebM 分别按实际轨道编码完成
  IPC 往返且 Worker 不退出；输出的时长、帧和接触表来自真实解码数据。
- 图片真实夹具测试：JPEG、PNG、WebP 依据内容签名解码；扩展名与内容不一致时以
  内容校验结果为准，不产生伪造特征。
