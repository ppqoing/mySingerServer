...
发布内容采用白名单：三个 Rust 可执行文件、五个固定 FFmpeg 运行 DLL、中心建库脚本和
许可证闭包。脚本不会复制 data、config、旧 Go/C++ 产物或任何 FFmpeg EXE。
...
try {
    ...
    # 使用 staging 的内容作为 ZIP 根目录，归档解压后直接得到三个 EXE、runtime、schema、
