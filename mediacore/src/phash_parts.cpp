...
void resize_to_96(const mc_image& source, float* destination) {
    ...
    for (int v = 0; v < kDctSize; ++v) {
        for (int u = 0; u < kDctSize; ++u) {
            ...
            for (int y = 0; y < kPartSize; ++y) {
                const float* row =
                    work + (base_y + y) * kWorkSize + base_x;
                double row_sum = 0.0;
                for (int x = 0; x < kPartSize; ++x) {
                    row_sum +=
                        row[x] * table.values[x * kDctSize + u];
                }
            ...
            coefficients[v * kDctSize + u] =
                static_cast<float>(0.25 * normalize_u * normalize_v * sum);
        }
    }
...
extern "C" MC_API int mc_phash_parts(
    ...
    if (out_parts != nullptr) {
        std::memset(out_parts, 0, sizeof(uint64_t) * MC_PHASH_PARTS);
    }
    try {
        if (image == nullptr || image->gray == nullptr || out_parts == nullptr) {
            set_error(errbuf, errbuf_len, "phase-2 pHash input or output is null");
            return MC_ERR_NULL_ARG;
        }
        if (image->width < 8 || image->height < 8) {
            set_error(errbuf, errbuf_len, "phase-2 pHash image is smaller than 8x8");
            return MC_ERR_SIZE;
        }
        ...
        std::memcpy(out_parts, result.data(), sizeof(result));
        return MC_OK;
    } catch (const std::bad_alloc&) {
        set_error(errbuf, errbuf_len, "out of memory computing phase-2 pHash");
        return MC_ERR_OOM;
    } catch (...) {
        set_error(errbuf, errbuf_len, "unexpected phase-2 pHash failure");
