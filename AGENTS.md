...
图片预览只在用户点击后由节点按 1 MiB 分块读取原文件，在 desktop 内存中解码并直接交给 Slint，
不写缓存、不生成缩略图；视频预览同样按需读取计算阶段已经生成的六帧 3×2 JPG 联系表，不做
额外抽帧。离线或失活
...
冻结输入、稳定组分页、复核恢复和删除后缩组测试也已通过；桌面管理 UI 的七项主导航、统一本地/
...
不增加回收站项目；测试项随后已精确还原并清理。改版前 Computer Use 能读取 desktop 总览与主导航标签、
